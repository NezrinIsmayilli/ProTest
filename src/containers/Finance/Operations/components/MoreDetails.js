    ...
    return (
      <div style={{ padding: 24, paddingBottom: 12 }}>
        ...
        <Row>
          ...
          />
          <Show title="Kateqoriya" subtitle={data.transactionType === 14 ? "İlkin qalıq" : data.categoryName} />
          ...
          {data.transactionType === 9 && data.creditId !== null && (
            <>
              ...
              <Show
                title="Faiz ödənişi"
                subtitle={`${parseFloat(data.creditAmount || 0).toFixed(2)} ${data.currencyCode
                  }`}
              />
              <Show
                title="Depozit ödənişi"
                subtitle={`${parseFloat(data.depositAmount || 0).toFixed(2)} ${data.currencyCode
                  }`}
              />
            </>
          )}
          ...
          {isLog ? (
            <Show
              ...
              subtitle={`${parseFloat(data.toAmount).toFixed(2)} ${data.toCurrency
                }`}
            />
          ) : (
