...
import RecPaySideBar from './recPaySideBar';
import RecievablesDetails from './RecievablesDetails';
import CurrenciesOption from './currenciesOption';
import queryString from 'query-string';
import { filterQueryResolver } from 'utils';
import { useHistory, useLocation } from 'react-router-dom';
import { fetchAllRecievables } from 'store/actions/export-to-excel/financeModule';
...
function Recievables(props) {
    ...
    const [tableSettingData, setTableSettingData] = useState(RECEIVABLES_TABLE_SETTING_DATA );
    ...
    const [filters, onFilter,setFilters] = useFilterHandle(
      ...
      if(typeof(filters['history'])=='undefined') {
          history.push({
            ...
            if (filters.currencyId) fetchRecievables(filters);
            if (filters.currencyId) fetchRecievablesCount(filters);
        }
    );
  ...
  useEffect(() => {
    ...
    //     };
    //     const data = addTotals(recievables).map((item, index) => [
    //         item.isTotal
  ...
  useEffect(()=>{
    fetchTableConfiguration({ module: 'Finance-Receivables' })
  },[]);
  ...
  useEffect(() => {
    if (tableConfiguration?.length > 0 && tableConfiguration !== null) {
      ...
      const column = RECEIVABLES_TABLE_SETTING_DATA 
    ...
    const handleSaveSettingModal = column => {
        ...
        createTableConfiguration({ moduleName: "Finance-Receivables", columnsOrder: data })
                            ...
                            <Col xl={24} xxl={24}>
                                ...
                                >
                                    ...
                                    <Can
                                        ...
                                        a={
                                            permissions.transaction_recievables_report
                                        }
                                    >
                                        ...
                                        <ExportToExcel
                                            ...
                                            excelTitle={`Debitor borclar`}
                                            excelName={`Debitor borclar (${
                                                currencies?.find(
                                                    currency =>
                                                        currency.id ===
                                                        selectedCurrency
                                ...
                                <Table
                                    ...
                                    dataSource={addTotals(recievables)}
...
const mapStateToProps = state => ({
    recievables: state.recievablesAndPayablesReducer.recievables,
    recievablesCount: state.recievablesAndPayablesReducer.recievablesCount,
