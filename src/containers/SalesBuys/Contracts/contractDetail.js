...
import BronInvoices from './operationDetails/bronInvoices';
...
function ConractDetail(props) {
    ...
    const [bronInvoice, setBronInvoice] = useState([]);

    useEffect(() => {
        if (row.id) {
            fetchOperationsList({
                filters: { contracts: [row.id], transactionTypes: [9, 8] },
                onSuccessCallback: ({ data }) => {
                    const result = data.filter(
                        item =>
                            item.transactionCatalogName !== null &&
                            (item.invoiceContractNo === null ||
                                row.contract_no === item.contractNo ||
                                item.contractNo === item.invoiceContractNo)
                    );
                    setFinExpenceData(result);
                },
        ...
        if (contract) {
            fetchSalesInvoiceList({
                filters: {
                    ...
                    isDeleted: 0,
                    invoiceTypes:
                        contract?.direction === 1 ? [1, 4, 10] : [2, 3],
                },
            });
        }
    }, [contract]);
            ...
            case 0:
                return (
                    <OpDetailTab
                        ...
                        contractId={row.id}
                        businessId={row.business_unit_id}
    ...
    return (
        ...
        >
            <div className={styles.detailsTab}>
                ...
                {row?.direction === 2 && bronInvoice.length > 0 ? (
                    <Button
                        style={{
                            borderRadius: 0,
                            borderRight: 0,
                    ...
                    >
                        Bron qaimələri({bronInvoice.length})
                    </Button>
                ) : null}
                {row?.direction === 2 ? (
                    <Button
