...
function OpFinOpInvoiceTab(props) {
  ...
  const columns = [
...

    {
      ...
      render: (value, row) =>
        `${formatNumberToLocale(defaultNumberFormat(formatNumberToLocale(
          defaultNumberFormat(
            mainCurrencyCode?.code === row.invoiceCurrencyCode
  ...
  useEffect(() => {
    if (filteredList.length > 0) {
      let tmp = 0;
      filteredList.map(
        ({ amountConvertedToMainCurrency, invoicePaymentAmountConvertedToInvoiceCurrency, invoiceCurrencyCode }) =>
                    (tmp =
                        Number(
                            mainCurrencyCode?.code === invoiceCurrencyCode
