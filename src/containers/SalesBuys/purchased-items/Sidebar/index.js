...
import { fetchPurchasedInvoices } from 'store/actions/operations/purchased-items';
...
const PurchasedItemsSidebar = ({
  ...
  const [selectedType, setSelectedType] = useState(filters.isSerialNumber?Number(filters.isSerialNumber):null);
  ...
  const [productCode,setProductCode]=useState(filters.productCode?filters.productCode:null);
  const [serialNumber,setserialNumber]=useState(filters.serialNumber?filters.serialNumber:null);
  const [barCode,setbarCode]=useState(filters.barcode?filters.barcode:null);
  ...
  const [
    filterSelectedPurchasedInvoices,
    setFilterSelectedPurchasedInvoices,
  ] = useState([]);
  const [
  ...
  const [filterSelectedSuppliers, setFilterSelectedSuppliers] = useState([]);
  const [filterSelectedCatalog, setFilterSelectedCatalog] = useState({root: []});
  const [
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
      if(filters.rootCatalogs &&catalogs.root?.length && filterSelectedCatalog.root?.length && !filtered){
       let catalogOptions=filterSelectedCatalog.root?.filter(catalog =>filters.rootCatalogs.map(id=>Number(id)).includes(catalog.id))
       let Options=catalogOptions.map(catalog=>{
         return {props:{catalog}}})
       handleParentCatalogsChange(filters.rootCatalogs.map(Number),Options)
       setFiltered(true)
      }
    ...
    useEffect(()=>{
     ...
     if(filters.productCatalogs&&subCatalogs.length){
       let sub=subCatalogs.filter(subCat=> filters.productCatalogs.map(Number).includes(subCat.id));
       let subOptions=sub.map(catalog=>{
        return {props:{catalog}}})
         
     handleChildCatalogsChange(filters.productCatalogs.map(Number),subOptions)
     }
     },[catalogs.children])
  ...
  useEffect(() => {
    ...
    if(filters.rootCatalogs) {
      fetchFilteredCatalogs({ids: filters.rootCatalogs.map(Number)}, data => {
        let appendList = {};
        if (data.data) {
    ...
    if (filters.invoices) {
        fetchPurchasedInvoices({ ids: filters.invoices.map(Number), invoiceTypes: [1] }, data => {
          const appendList = [];
          ...
          setFilterSelectedPurchasedInvoices(appendList)
      });
    }
    if (filters.salesman) {
      fetchUsers({
    ...
    if (filters.suppliers) {
      const filter = { ids: filters.suppliers.map(Number) , categories: [4] };
      fetchContacts(filter, data => {
          ...
          setFilterSelectedSuppliers(appendList);
      });
    }
...
const ajaxInvoicesSelectRequest = (
  ...
  const filters = { limit, page, name: search, invoiceTypes: [1] };
  fetchPurchasedInvoices(filters, data => {
      const appendList = [];
      ...
      if (stateReset) {
          setPurchasedInvoices(appendList);
      } else {
          setPurchasedInvoices(purchasedInvoices.concat(appendList));
      }
  });
};
    ...
    <Sidebar title="Ticarət">
      ...
      <ProSidebarItem label="Anbar">
          <ProAsyncSelect
              ...
              valueOnChange={stocks =>{handlePaginationChange(1); onFilter('stocks', stocks)}}
              value={filters.stocks ? filters.stocks.map(Number) : undefined}
              data={
      ...
      <ProAsyncSelect
                ...
                valueOnChange={newCatalogs => {
                    ...
                    setSelectedCatalogs([...selectedCatalogs, ...catalogs.root
                      ?.filter(catalog =>
                          newCatalogs.includes(catalog.id)
                      ).filter(d => !new Set(selectedCatalogs.map(de => de.id)).has(d.id))])
                    ...
                    handleParentCatalogsChange(
                        newCatalogs,
                        [...selectedCatalogs, ...catalogs.root?.filter(d => !new Set(selectedCatalogs.map(de => de.id)).has(d.id))]?.filter(catalog =>
                                newCatalogs.includes(catalog.id)
                            )
                            .map(catalog => ({ props: { catalog } }))
                    );
      ...
      <ProSidebarItem label="Alt kataloq">
        <Select
          // loading={isLoading}
          value={filters.productCatalogs?
            filters.productCatalogs.map(Number):childCatalogs.map(childCatalog => childCatalog.id)}
          onChange={(newCatalogs, options) =>
      ...
      <ProSidebarItem label="Məhsul adı">
          <ProAsyncSelect
              ...
              valueOnChange={products =>{handlePaginationChange(1); onFilter('products', products)}}
              data={
              ...
              value={filters.products?filters.products.map(Number):undefined}
          />
      </ProSidebarItem>
      ...
      <ProSidebarItem label="Təchizatçı">
          <ProAsyncSelect
              mode="multiple"
              selectRequest={ajaxSupplierSelectRequest}
              valueOnChange={clients => {
                handlePaginationChange(1);
                onFilter('suppliers', clients)
              }}
              data={
                filterSelectedSuppliers.length > 0
                    ? [
              ...
              value={filters.suppliers?filters.suppliers.map(Number):undefined}
          />
      </ProSidebarItem>
      <ProSidebarItem label="Müqavilə">
          <ProAsyncSelect
              ...
              valueOnChange={contracts =>{ 
                ...
                value={filters.contracts?filters.contracts.map(Number):undefined}
          />
      </ProSidebarItem>
      <ProSidebarItem label="Qaimə">
          <ProAsyncSelect
              ...
              valueOnChange={invoices =>{ 
                ...
                data={
                  filterSelectedPurchasedInvoices.length > 0
                      ? [
                            ...filterSelectedPurchasedInvoices,
                            ...purchasedInvoices.filter(
                                item =>
                                    !filterSelectedPurchasedInvoices
                                        .map(({ id }) => id)
                                        ?.includes(item.id)
      ...
      <ProSidebarItem label="Satış meneceri">
          <ProAsyncSelect
              ...
              value={filters.salesman?filters.salesman.map(Number):undefined}
          />
      </ProSidebarItem>
      <ProSidebarItem label="Agent">
          <ProAsyncSelect
              ...
              value={filters.agents?filters.agents.map(Number):undefined}
          />
      </ProSidebarItem>
...
const mapStateToProps = state => ({
  ...
  purchasedInvoices: state.purchasedItemsReducer.purchasedInvoices,
